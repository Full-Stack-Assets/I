...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === '/v1/email-report' && request.method === 'POST')
        return cors(await handleEmailReport(request, env), env, origin);
...
function reportHtml(a) {
  const clauses = Array.isArray(a.flagged_clauses) ? a.flagged_clauses : [];
  const rows = clauses.map(c =>
    '<div style="border-left:4px solid #9333ea;padding:8px 12px;margin:8px 0;background:#faf5ff">'
    + '<strong>[' + escapeHtml((c.severity || '').toUpperCase()) + '] ' + escapeHtml(c.title) + '</strong> '
    + '<span style="color:#888">(' + escapeHtml(c.section_reference || '—') + ')</span><br>'
    + escapeHtml(c.explanation) + '<br><em>What to do:</em> ' + escapeHtml(c.recommendation) + '</div>'
  ).join('');
  return '<h2>ContractScan — risk assessment</h2>'
    + '<p><strong>Risk score:</strong> ' + escapeHtml(a.risk_score) + '/100 (' + escapeHtml(a.risk_level) + ')</p>'
    + '<p>' + escapeHtml(a.summary) + '</p>' + (rows || '<p>No flagged clauses.</p>')
    + '<p style="color:#888;font-size:12px">Informational only — not legal advice. Generated by ContractScan.</p>';
}
async function handleEmailReport(request, env) {
  const ip = request.headers.get('CF-Connecting-IP') || 'anon';
  if (!(await rateLimit(env, 'rl:email:' + ip + ':' + Math.floor(Date.now() / 3600000), 5, 3600)))
    throw httpError('Too many emails. Try again later.', 429);
  if (!env.RESEND_API_KEY) throw httpError('Email is not configured on this deployment.', 503);

  const { to, analysis } = await request.json().catch(() => ({}));
  if (!EMAIL_RE.test(to || '')) throw httpError('Enter a valid email address.', 400);
  if (!analysis || typeof analysis !== 'object') throw httpError('Missing report data.', 400);

  const sent = await sendEmail(env, to, 'Your ContractScan report', reportHtml(analysis));
  if (!sent) throw httpError('Could not send the email. Try again.', 502);
  return json({ ok: true });
}
...
// Generic email send via Resend (https://resend.com). Returns true on success.
async function sendEmail(env, to, subject, html) {
  ...
  try {
    const r = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from: env.FROM_EMAIL || 'ContractScan <onboarding@resend.dev>', to, subject, html })
    });
...
// Emails the unlock code to a buyer after purchase.
async function sendCodeEmail(env, to, code, credits) {
  ...
  return sendEmail(env, to, 'Your ContractScan unlock code', html);
}
